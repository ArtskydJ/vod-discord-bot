...
// - 5 so it lines up with the current rotation
var userIndex = Math.round(now.valueOf() / (1000 * 60 * 60 * 24) * 6 / 7) - 5
...
client
	.login(config.token)
	.then(DEBUG ? debugging : postMessage)
...
function debugging() {
	var todayUser = config.users[userIndex % config.users.length]
	var message = 'It is ' + todayUser.name + '\'s turn to post a verse today!'
	console.log(message)
	process.exit(0)
}
